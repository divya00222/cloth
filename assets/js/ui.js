...
const UIManager = {
  ...
  initWhatsAppCTA: () => {
    ...
    const phone = (typeof NEPAL_CONFIG !== 'undefined') ? NEPAL_CONFIG.contact.whatsappNumber : '9779801234567';
    const msg = encodeURIComponent((typeof NEPAL_CONFIG !== 'undefined') ? NEPAL_CONFIG.contact.whatsappMessage : 'Hello AURA Studios');
    ...
    btn.innerHTML = `
      <span class="whatsapp-float-tooltip">Chat with Atelier (+977 9801234567)</span>
  ...
  renderCampaignBanner: () => {
    ...
    if (typeof NEPAL_CONFIG === 'undefined') return;

    const activeCamp = NEPAL_CONFIG.getActiveCampaign();
    const allCampaigns = NEPAL_CONFIG.campaigns;
    ...
    bannerContainer.innerHTML = `
      ...
      <div class="campaign-switcher-bar">
        ...
        ${Object.keys(allCampaigns).map(key => {
          ...
          return `
            <button class="campaign-switch-btn ${isActive ? 'active' : ''}" onclick="NEPAL_CONFIG.setActiveCampaign('${c.key}')">
              ${c.name}
            </button>
          `;
        }).join('')}
